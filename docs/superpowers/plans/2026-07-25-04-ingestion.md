    ...
    TimeProvider timeProvider) : IRequestHandler<SyncCommand, SyncJobDto>
{
    ...
    /// <summary>
    /// 自動建立的品類**必須**宣告 provider 會寫入的 attributes key。
    /// MongoItemSyncWriter 直接寫 BsonDocument 繞過驗證，所以同步本身會成功；
    /// 但 AttributeValidator 會拒絕任何未宣告的 key，而 UpdateItemCommandHandler
    /// 每次更新都重新驗證整包 attributes——若這裡留空，使用者要把同步進來的遊戲
    /// 設為精選就會拿到 400，而「設為精選」正是 Showcase 圖片延遲下載的唯一入口，
    /// 整個功能會變成無法觸發。這些 key 對應 SteamProvider.ToExternalItem 的輸出。
    ///
    /// 三者都不可設 Required：iconUrl 只在 Steam 提供 icon 時才存在。
    /// </summary>
...
- [ ] **Step 5b: 立刻註冊佇列（不可延到 Task 10）**

`UpdateItemCommandHandler` 是 MediatR handler，多了 `IShowcaseImageQueue` 這個相依之後，`ValidateOnBuild` 會讓**所有**用到 `ApiFactory` 的整合測試在啟動階段失敗。`src/MyCollection.Infrastructure/DependencyInjection.cs` 追加（補 `using MyCollection.Application.Showcase;`、`using MyCollection.Infrastructure.Imaging;`）：

```csharp
        services.AddSingleton<IShowcaseImageQueue, ShowcaseImageQueue>();
```

**這一步只註冊佇列本身**。`AddHostedService<ShowcaseImageDownloader>()` 與它專用的具名 `HttpClient` 留給 Task 10——背景服務會在整合測試中真的啟動，等 Task 10 一起處理比較乾淨。
...
- [ ] **Step 6b: 跑完整 `dotnet test`**

Expected: `Failed: 0`（若整合測試整批失敗，代表 Step 5b 沒做）
