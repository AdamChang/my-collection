...
public sealed class MongoItemSyncWriter(MongoContext context) : IItemSyncWriter
{
        ...
        CancellationToken ct)
    {
        ...
        catch (MongoBulkWriteException<Item> ex)
        {
            ...
            return new SyncOutcome(upserts, (int)ex.Result.ModifiedCount, ex.WriteErrors.Count);
        }
        ...
        return new SyncOutcome(result.Upserts.Count, (int)result.ModifiedCount, 0);
    }
        ...
        DateTime syncedAt)
    {
        ...
        {
            { "name", item.Name },
            { "externalRef.url", item.SourceUrl?.ToString() ?? BsonNull.Value.ToString() },
        ...
        {
            set[$"attributes.{key}"] = BsonValue.Create(value) ?? BsonNull.Value;
        }
...
`externalRef.url` 在無來源 URL 時應存 BSON null 而非字串。把該行改為在 `set` 建好後條件寫入：

```csharp
        set["externalRef.url"] = item.SourceUrl is null ? BsonNull.Value : item.SourceUrl.ToString();
```

（移除初始化器中的那一行 `{ "externalRef.url", ... }`。）
