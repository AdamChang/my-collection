...
日期：2026-07-28
狀態：**已於 2026-08-03 取代，見 `2026-08-03-image-transfer-design.md`**

> 這份設計解決的是「兩台各自獨立部署的機器之間搬移收藏資料」。改用 MongoDB Atlas 之後兩台連的是同一個叢集，資料本來就同一份，此問題不再存在，功能已收窄為只搬圖片。以下內容保留作為決策脈絡，不再反映實作現況。
